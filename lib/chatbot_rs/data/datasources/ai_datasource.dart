...
class AIService {
  ...
  static Future<Map<String, dynamic>> classifyIntent(String text) async {
    const url = 'https://api-inference.huggingface.co/models/indobenchmark/indobert-base-p1';
  ...
  static Future<String> generateResponse(List<Map<String, String>> messages) async {
    ...
    // List of free models to try sequentially if one fails (e.g. 429 Rate Limit)
    ...
    if (_openRouterApiKey == 'YOUR_OPENROUTER_API_KEY' || _openRouterApiKey.isEmpty) {
    ...
    final fullMessages = [systemPrompt, ...messages];

    for (String model in freeModels) {
      try {
        final response = await http.post(
          Uri.parse(url),
          headers: {
