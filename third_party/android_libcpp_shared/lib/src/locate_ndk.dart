  ...
  static Future<NDKInfo> _getNDKInfo(Uri ndkPath, {Logger? logger}) async {
    ...
    for (final host in hostArchitectures) {
      final sysrootLibDir = Directory(
        host.llvmToolchainPath.resolve('sysroot/usr/lib/').toFilePath(),
      );
    ...
    for (final host in hostArchitectures) {
      for (final target in host.targetArchitectures) {
        final targetLibDir = Directory(target.sysrootLibPath.toFilePath());
  ...
  NDKInfo? forBuildConfig(BuildConfig config) {
    ...
    for (final ndk in sorted) {
      ...
      if (host != null) {
        ...
        if (target != null) {
          final sharedCpp = File(
            target.sysrootLibPath.resolve('libc++_shared.so').toFilePath(),
          );
          if (!sharedCpp.existsSync()) {
            continue;
          }
